...
const dependentsOfDependency = (sourceFiles, file) => {
  ...
  if (sourceFilesThatImportFile.length) {
    const files = flatMap(sourceFilesThatImportFile, (sourceFileThatImportFile) => {
...
export default async ({ source, target, file, environment = "production" }) => {
  ...
  const sourceFilesThatCouldBeBuilt = dependentsThatAreNotDependencies(sourceFiles)
  ...
  for (const sourceFile of sourceFilesToBuild) {
    ...
    try {
      ...
      logger.builder.building(`Error building to "${sourceFile.outputPath}: ${error.message}`)
