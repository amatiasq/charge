...
export class Logger {
  constructor({ groups, disabled = false }) {
    ...
    Object.entries(groups).forEach(([group, actions]) => {
      this[group] = Object.entries(actions).reduce(
        (object, [actionKey, actionOptions]) => {
          object[actionKey] = message =>
            this._log({ actionOptions, group, message })
          return object
        },
        {},
      )
  ...
  get _longestLabelLength() {
    const actions = Object.values(this._groups)
    const actionOptions = flatMap(actions, action => Object.values(action))
    const labels = actionOptions.map(({ label }) => label)
  ...
  _log({ actionOptions, group, message }) {
    ...
    const { label, labelColor } = actionOptions

    console.log(
      [
        this._group(group),
        actionOptions.icon,
        this._label(label, labelColor),
        message,
      ]
        .filter(Boolean)
        .join(' '),
  ...
  _label(label, labelColor) {
    const padding = this._longestLabelLength - label.length

    label = labelColor
      ? chalk[labelColor].underline(label)
